...
const APPROACH_THRESHOLD: f32 = 0.1; // 20% of the boundary size
...
pub fn wall_approach_system(
    ...
    let threshold_distance = boundary_size * APPROACH_THRESHOLD;
        ...
        if let Some(collision_point,) = boundary.find_edge_point(position, direction,) {
            ...
            if distance_to_wall <= threshold_distance {
                ...
                if teleporter.just_teleported {
                    ...
                    if emerging.distance > threshold_distance {
                        visual.emerging = None;
                    } else {
                        let progress = emerging.distance / threshold_distance;
